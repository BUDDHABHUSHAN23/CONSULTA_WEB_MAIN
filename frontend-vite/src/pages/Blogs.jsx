  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {filtered.length === 0 ? (
            <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center text-sm text-gray-600">No posts found.</div>
          ) : (
            <div className="grid gap-4 sm:gap-6 [grid-template-columns:repeat(auto-fit,minmax(260px,1fr))]">
              {filtered.map((p) => (
                <article key={p.slug} className="group rounded-2xl border border-gray-200 bg-white p-5 sm:p-6 hover:shadow-2xl transition-all">
                  <header>
                  ...
                  <div className="mt-3 flex items-center gap-3 text-xs text-gray-600">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {(p.tags || []).slice(0, 3).map(t => (
                        <span key={t} className="inline-flex items-center gap-1 rounded-full bg-gray-100 px-2 py-0.5"><Tag className="h-3 w-3" /> {t}</span>
                      ))}
                    </div>
                  </div>
                  <div className="mt-4">
                    <Link to="/contact" className="text-sm text-gray-900 underline-offset-2 hover:underline">Read more →</Link>
                  </div>
                </article>
              ))}
            </div>
